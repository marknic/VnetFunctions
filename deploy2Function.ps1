...
$deployResult = $(az deployment group create -g $resourceGroupName --template-file '.\createPremiumFunction.bicep' --parameters params.functionapp.json)

if (!$?) {
  Write-Error "Error deploying the createPremiumFunction.bicep template.  Exiting"
  Exit
}
