...
// Original script:  https://github.com/Azure/azure-quickstart-templates/blob/master/quickstarts/microsoft.web/function-app-storage-private-endpoints/main.bicep
...
param functionSubnetAddressPrefix string = '10.3.0.0/24'

@description('The IP address space used for the private endpoints.')
param privateEndpointSubnetAddressPrefix string = '10.3.1.0/24'
...
// // -- Private DNS Zones --
var dnsPzData = {
  network: virtualNetwork.id
  zones: [
    {
      resourceName: 'storageFileDnsZone'
      zoneName: privateStorageFileDnsZoneName
      linkName: '${storageFileDnsZone.name}-link'
    }
    {
      resourceName: 'storageBlobDnsZone'
...
module storageFileDnsZone 'modules/dnsZone.bicep' = {
  name: 'storageFileDnsZone'
  params: {
    zoneName: privateStorageFileDnsZoneName
  }
}

module storageFileLink 'modules/dnsZoneLink.bicep' = {
  name: 'storageFileLink'
  params: {
    name: '${storageFileDnsZone.name}-link'
    parent: storageFileDnsZone.outputs.zoneId
    virtualNetworkId: virtualNetwork.id
  }
}

var dnsZoneName = 'storageBlobDnsZone'

module dnsZoneResource 'modules/dnsZone.bicep' = {
  name: dnsZoneName
  params: {
    zoneName: privateStorageBlobDnsZoneName
  }
...
module storageBlobLink 'modules/dnsZoneLink.bicep' = {
  name: 'storageBlobLink'
  params: {
    ...
    privateDnsZoneConfigs: [
      {
        name: 'config'
        properties: {
          privateDnsZoneId: storageFileDnsZone.outputs.zoneId
        }
      }
    ]
  }
